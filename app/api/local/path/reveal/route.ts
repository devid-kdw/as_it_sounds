...
import { revealLocalTokenizedPath } from "@/lib/local-export";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await revealLocalTokenizedPath(parsed.data.tokenizedPath);
    return NextResponse.json({ ok: true, data: result });
  } catch (error) {
    return localActionErrorResponse(error, "local_reveal_failed", "Unable to reveal local path.");
  }
}
